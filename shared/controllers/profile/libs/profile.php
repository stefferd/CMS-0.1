...
class ProfileController {
    ...
    public function checkLogin($formvars = array()) {
        ...
        if ($this->profile->getId() != 0) {
            ...
            $_SESSION['LOGIN'] = true;
            echo '<div class="blocks login">U bent ingelogd, <a href="' . str_replace('index.php', '', $_SERVER['PHP_SELF']) . '">Ga terug naar startpagina</a>.</div>';
        } else {
            $_SESSION['username'] = '';
            $_SESSION['id'] = 0;
            $_SESSION['fullname'] = '';
            $_SESSION['LOGIN'] = false;
            echo '<div class="blocks login">U bent niet ingelogd, <a href="' . str_replace('index.php', '', $_SERVER['PHP_SELF']) . '">Ga terug naar startpagina en probeer het opnieuw.</a></div>';
        }
    }
