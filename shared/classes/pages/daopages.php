...
class DaoPages implements IntDaoPages
{
    ...
    public function getByUniqueId(Pages $pages, $uniqueid) {
        $db = new Db();
        if ($smtm = $db->mysqli->prepare('SELECT id, uniqueid, title, text, metatitle, metadescription, keywords, active, created, updated, user, version FROM pages WHERE uniqueid = ?')) {
